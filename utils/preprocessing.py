...
def decode_results(mask, num_images=1, num_classes=2):
  """Decode batch of segmentation masks.
  ...
  assert (n >= num_images), 'Batch size %d should be greater or equal than number of images to save %d.' \
                            % (n, num_images)
  outputs = np.zeros((num_images, h, w, 256), dtype=np.uint8)#64
  for i in range(num_images):
    img = mask[i]
#     pixels = img.load()
    outputs[i] = np.array(img)
...
def decode_net(mask, net_num,num_images=1, num_classes=2):
  """Decode batch of segmentation masks.
  ...
  assert (n >= num_images), 'Batch size %d should be greater or equal than number of images to save %d.' \
                            % (n, num_images)
  outputs = np.zeros((num_images, h, w, net_num), dtype=np.uint8)#64
  for i in range(num_images):
    img = mask[i]
#     pixels = img.load()
    outputs[i] = np.array(img)
...
def decode_class(class_tensor,num_images=1):
    
    n,l = class_tensor.shape
    assert(n>=num_images)
    outputs = np.zeros((num_images,l),dtype=np.uint8)
    for i in range(num_images):
        c = np.zeros(3)    # 3     
        idx = np.argwhere(class_tensor[i] == np.max(class_tensor[i]))[0][0]
        c[idx] = 1
        outputs[i]=c
    return outputs

def gt_class_label(gt_class,num_images=1):    
    n,l = gt_class.shape
#     assert(n>=num_images), 'Batch size %d should be greater or equal than number of images to save %d.' \
#                             % (n, num_images)
    class_labels = np.zeros(n,dtype=np.int64)
    for i in range(n):
        idx = np.argwhere(gt_class[i] == np.max(gt_class[i]))[0][0]
        class_labels[i]=idx
    return class_labels

def decode_end_points(end_points):
    n,l = class_tensor.shape
    assert(n>=num_images)
    outputs = np.zeros((num_images,l),dtype=np.uint8)
    for i in range(num_images):
...
def decode_pred_class(class_tensor):
    outputs = np.zeros(3,dtype=np.uint8) # 3
#     c = np.zeros(3)        
    idx = np.argwhere(class_tensor == np.max(class_tensor))[0][0]
    outputs[idx] = 1
    return outputs
...
#     image = tf.to_float(tf.reshape(image, shape=[128,128,256]))
...
#       label = tf.to_int32(tf.reshape(label, shape=[512,512,1]))
...
def mask_input_fn(net_c1,net_c2,net_c3, net, batch_size=1):
  def read_np(filename):
    n = np.load(str(filename, encoding = "utf-8"))
    n = n.astype(np.uint8)
    return n
  def _parse_function(input_nets, is_label):
    
    net_c,net = input_nets
    net_c1,net_c2,net_c3 = net_c
    net_c1 = tf.py_func(read_np,[net_c1], tf.uint8)
    net_c2 = tf.py_func(read_np,[net_c2], tf.uint8)
    net_c3 = tf.py_func(read_np,[net_c3], tf.uint8)
    net = tf.py_func(read_np,[net], tf.uint8)
#     net_c = tf.read_file(net_c)
#     net = tf.read_file(net)
#     net_c = tf.decode_raw(net_c,tf.uint8)    
#     net = tf.decode_raw(net,tf.uint8)
    net_c1 = tf.to_float(tf.reshape(net_c1,shape=[64,64,256]))
    net_c2 = tf.to_float(tf.reshape(net_c2,shape=[32,32,512]))
    net_c3 = tf.to_float(tf.reshape(net_c3,shape=[32,32,1024]))
    net = tf.to_float(tf.reshape(net,shape=[32,32,2048]))
    nets = {
        'net_c1':net_c1,
        'net_c2':net_c2,
        'net_c3':net_c3,
...
#   net_c = np.array(net_c).flatten()#.astype(np.uint8).tostring()
#   net = np.array(net).flatten()#.astype(np.uint8).tostring()
  net_c = (net_c1,net_c2,net_c3)
  input_nets = (net_c, net)

  dataset = tf.data.Dataset.from_tensor_slices(input_nets)
  dataset = dataset.map(lambda x, y: _parse_function((x, y), True))
  dataset = dataset.prefetch(batch_size)
  dataset = dataset.batch(batch_size)
  iterator = dataset.make_one_shot_iterator()

  nets = iterator.get_next()
